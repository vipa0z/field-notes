...
### parent child
parent-child  inlanefrieght (parent) trusts <->corp.ad.someorg.local (child)
...
- `Bidirectional trust`: Users from both trusting domains can access resources in the other domain. For example, in a bidirectional trust between `ad.someorg.local` and `FREIGHTLOGISTICS.LOCAL`, users in `ad.someorg.local` would be able to access resources in `FREIGHTLOGISTICS.LOCAL`, and vice-versa.