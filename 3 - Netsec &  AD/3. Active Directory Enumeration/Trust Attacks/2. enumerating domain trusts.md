...
SourceName      : ad.someorg.local
TargetName      : LOGISTICS.ad.someorg.local
...
```powershell-session
 Get-DomainUser -Domain LOGISTICS.ad.someorg.local | select SamAccountName
...
```
C:\htb> netdom query /domain:ad.someorg.local dc
...
```cmd
C:\htb> netdom query /domain:ad.someorg.local workstation
