...
```powershell-session
PS C:\htb> ls \\academy-ea-dc01\SYSVOL\ad.someorg.local\scripts

    Directory: \\academy-ea-dc01\SYSVOL\ad.someorg.local\scripts
...
```powershell-session
PS C:\htb> cat \\academy-ea-dc01\SYSVOL\ad.someorg.local\scripts\reset_local_admin_pass.vbs
...
1. dump sysvol directory tree, grep for xml
```
smbclient //172.16.8.3/SYSVOL -U 'ad.someorg.local\hporter%Gr8hambino!' -c "recurse; ls" > sysvol_smb_results
cat sysvol_smb_results |grep '.xml'
```
