...
1. In Servers hosting web services:
once initial access is obtained on a web server host, the most straightforward path is to hunt for DB files
- [ ] grep for db connection strings, Look for DBs  And Dump users.
...
- [ ]  Get **OS information**
- [ ] Kernel version, look for recent kernel exploits like netfilter (they work better and sometimes pre-compiled)
- [ ] Running Services
...
- [ ] list processes and bound ports
```
ps -ef --forest |less -S
```
